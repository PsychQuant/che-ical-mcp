...
- **`EventKitManaging` protocol + `FakeEventKitManager` + `CleanupHandlerTests`** (#31): protocol-oriented test harness for handler integration tests. `EventKitManaging` exposes the 3 methods `handleCleanupCompletedReminders` depends on; `CheICalMCPServer.init(reminderCleanupSource:)` accepts an injection with `EventKitManager.shared` as the default (production unchanged). `FakeEventKitManager` actor scripts return values and records invocations. `CleanupHandlerTests` (11 new cases) pins #21 F1 guard ordering, #21 F2 dedupe / R2-F2 arithmetic, #28 F1 `onlyCompleted` enforcement, #21 F4 limit cap, #21 F8 response shape stability, and the binding-vs-filter mode branching — exactly the gaps Codex had to catch by reading code + schema during verification. Narrow scope per `/spectra-discuss` convergence: protocol only covers methods the cleanup handler uses; other 30+ EventKitManager methods stay direct-singleton and will be protocol-fied on demand.

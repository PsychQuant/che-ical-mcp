...
### Documentation
- **`writeFailureLog` + R3 inline-write thread-safety note (#70)**: documented the empirical concurrency guarantee for the 11 `FileHandle.standardError.write` sites in the sanitizer cluster — macOS POSIX `write(2)` is atomic for byte counts ≤ `PIPE_BUF` (4096 on macOS); typical post-cap + post-escape failure-line emission stays well under 4KB (handler/identifier are short tags; `safeRawLog` is bounded by `maxRawLogChars ≈ 1024 chars × ~3 bytes worst-case for escape-inflated UTF-8`). Concurrent `async` failing-tool-call invocations therefore do not interleave at the kernel level under normal failure-line shapes. A serial actor (`StderrLogger.shared`) was considered (Option A) but deferred — the empirical guarantee is sufficient at this server's concurrency profile. **Trigger to revisit**: any future-line shape exceeding ~3KB (structured stderr, stack traces, `#66` periodic-summary mechanism) breaks the PIPE_BUF safety net. See `#70` closing summary for full deferral rationale.

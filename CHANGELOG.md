...
### Hardening (Cluster B — release-pipeline robustness)

- **`scripts/sign-and-notarize.sh` post-notarize spctl cross-check** (#53): after `xcrun notarytool submit --wait` returns success, an additional `[4/5]` step runs `spctl -a -vvv -t install $BINARY` and refuses to exit 0 unless the output matches `source=Notarized Developer ID`. Catches the partial-state race where notarytool reports success but Apple's CDN hasn't propagated the verdict — without this check, the binary would be packed into the .mcpb in a "signed but Gatekeeper-rejects" state and only fail on user first launch.
- **`scripts/build-mcpb.sh` pre-pack codesign re-check** (#53): when `REQUIRE_CODESIGN=1` is set, the script now verifies the universal binary at `mcpb/server/CheICalMCP` actually carries an `Authority=Developer ID` signature before proceeding to the `mcpb pack` step. Belt-and-suspenders against `sign-and-notarize.sh` exit-code loss in piped/CI invocations — `make release-signed` already sets `REQUIRE_CODESIGN=1`, so the canonical release path will fail-fast on missing signature instead of silently shipping an unsigned `.mcpb`.
- **`make verify-release-ready` target** (#48): new informational pre-flight that compares `AppVersion.current` against `git tag --sort=-creatordate | head -1`. Three outcomes — match (no version bump needed for next release), source ahead by one (expected pre-release work), or unrelated divergence (warning to either tag now or revert the bump). `release-signed` now depends on this target so the warning surfaces every release-cut. Soft check — never aborts (a maintainer cutting v1.7.2 is expected to have AppVersion ahead of v1.7.1 tag mid-flight).

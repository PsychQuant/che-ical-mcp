...
> **Umbrella note (#166 display_name sweep)**: a sweep of all 15 che-mcps `mcpb/manifest.json` files found one other server with a `&` in `display_name` — **che-duckdb-mcp** (`"DuckDB Documentation & Database"`). Not Desktop-installed → not user-impacting yet, but would drop identically if installed under Desktop 1.18286.0. Fix rides along on its next release; tracked as a sibling follow-up.
...
> **Umbrella note (#166 sister sweep)**: the same `serverInfo.name` ≠ manifest-id mismatch exists in che-apple-notes-mcp (`CheAppleNotesMCP`) and che-xcode-mcp (`CheXcodeMCP`), and likely che-contacts-mcp (`CheContactsMCP`). **This is now moot for the Desktop-drop symptom** — the mismatch is refuted as the cause (the actual cause is the `display_name` `&`, above). These alignments remain worthwhile hygiene and ride along on each server's next release, but they are NOT a Desktop-injection fix; the sibling to actually watch for the Desktop-drop bug is **che-duckdb-mcp** (the `&`-in-`display_name` carrier).

...
**Invariant: the value passed as `Server(name:)` (i.e. the runtime `serverInfo.name`) MUST equal `mcpb/manifest.json` `name`.** Both are `che-ical-mcp` (kebab). The **leading hypothesis** for #166 (n=4 correlation across local MCP servers; **Desktop-side unproven** — the notarized-reinstall confirmation is blocked on the expired Apple agreement) is that Claude Desktop 1.18286.0's tool-injection layer reconciles the running server's self-reported `serverInfo.name` against the manifest / extension id and, on mismatch, silently drops the **entire** server (all tools) from conversations — the transport handshake and `tools/list` still complete, so no log surfaces the drop. che-ical vanished from Desktop while still working in Claude Code (which does not reconcile the names). **Even if that exact Desktop mechanism is never confirmed, a server's `serverInfo.name` matching its manifest id is a baseline MCP expectation — so this invariant stands on its own.**
...
> **Umbrella note (#166 sister sweep)**: the same mismatch is **confirmed** in che-apple-notes-mcp (`CheAppleNotesMCP`) and che-xcode-mcp (`CheXcodeMCP`), and **likely** in che-contacts-mcp (`CheContactsMCP`, whose manifest was not resolvable during the sweep — its serverInfo is PascalCase but the manifest side is unconfirmed). None are Desktop-installed yet, so none are user-impacting, but any would risk the same silent drop if installed as `.mcpb` under Desktop 1.18286.0. Fix rides along on each server's next release; tracked separately.

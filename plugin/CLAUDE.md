...
- **troubleshoot-tcc** — TCC permission diagnostic walkthrough. Use when user reports calendar tools failing silently / permission denied / missing dialog after upgrade. Wraps the v1.9.0 `--print-tcc-path` diagnostic flag into a 6-step fix flow (covers both the binary layer and the host-app / responsible-process layer, #168).
...
- **Two-layer authorization (#168)**: besides the binary's own grant, macOS TCC may attribute access to the **responsible process** (host app). Claude Desktop spawns `.mcpb` servers via `Helpers/disclaimer` → the `.mcpb` binary is its own TCC client (one layer). Claude Code spawns `~/bin/CheICalMCP` directly, no disclaim → attribution follows the host context, whose System Settings entry shows as a bare version number (e.g. `2.1.202` = `~/.local/share/claude/versions/<version>`). Operative rule: keep BOTH the `CheICalMCP` entry AND the host-layer entry ON for the host you use — both-ON is a **sufficient** condition; whether the binary layer is *also* strictly necessary (AND-gated vs. only the responsible-process layer mattering) is not conclusively established (#168), so this says "keep both ON", not "both are required"
- **`EKEventStore.authorizationStatus(for:)` is context-dependent (#168)**: it reports the attribution of the *calling context*, not an absolute property of the binary — two different binaries return identical status from the same Claude Code shell. Diagnose a host's tool calls from that host's own context (Claude Code → its Bash tool; Desktop → the startup-banner lines in its MCP log)
- **Claude Code updates rotate the versioned binary path** → the host-layer grant can silently go stale after an update (#170); re-check the version-number entry when calendar tools break right after a Claude Code upgrade

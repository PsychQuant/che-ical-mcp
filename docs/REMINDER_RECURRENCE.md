...
Repeated completion requests preserve the existing completion instant (#211). Completion records retain both the pre-write instant for undo and post-write instant for redo (#212). If saving advances the recurring object to an incomplete successor, the record retains the instant written before save; the existing occurrence guard still prevents replay onto a different occurrence.
